...
import type {NextApiResponseError, NextApiResponseSuccess} from "~/interfaces/api";
...
export async function GET(request: NextRequest) {
    try {
        ...
        // Forward all search params from the client to the FastAPI service
        for (const [key, value] of searchParams.entries()) {
            fastapiUrl.searchParams.set(key, value);
        }
        fastapiUrl.searchParams.set('query', ""); // Add required params if not present
        ...
        if (!fastapiResponse.ok) {
            ...
            console.error(`FastAPI fetch failed with status ${fastapiResponse.status}: ${errorBody}`);
             return NextResponse.json(
                 { success: false, error: `FastAPI error: ${fastapiResponse.statusText}` } as NextApiResponseError,
